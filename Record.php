...
// Note: SQL queries are hand-crafted, as the Framework prepared statements seem to
...
class Record
{
     ...
     * @return array An associative array (field_name => instance => value). In a non-repeating context, instance = 1
     * @throws Exception for violations of the project data structure.
     */
    public function getFieldValues($fields, $event = null, $instances = 1) {
        // Validate input.
        if (!is_array($instances)) $instances = array($instances);
        if (!is_array($fields)) $fields = array($fields);
        $event_id = $this->project->getEventId($event);
        $mode = $this->validateFields($fields, $event_id, $instances);
